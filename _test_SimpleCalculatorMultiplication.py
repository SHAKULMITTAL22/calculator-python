# test_SimpleCalculatorMultiplication.py

import pytest
from calculator import SimpleCalculator  # Ensure the correct import path

class TestSimpleCalculatorMultiplication:

    def test_multiplication_with_none_values(self):
        with pytest.raises(TypeError):
            SimpleCalculator.multiplication(None, 3)

    def test_multiplication_with_list_values(self):
        with pytest.raises(TypeError):
            SimpleCalculator.multiplication([1, 2], 3)

    def test_multiplication_with_tuple_values(self):
        with pytest.raises(TypeError):
            SimpleCalculator.multiplication((1, 2), 3)

    def test_multiplication_with_dict_values(self):
        with pytest.raises(TypeError):
            SimpleCalculator.multiplication({'key': 'value'}, 3)
